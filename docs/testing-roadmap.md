...
| **`validateFilterList`** | [src/filter/validate.test.ts](../src/filter/validate.test.ts) | ✅ | 8 |
| **`useFilteredChatBuffer`** | [src/hooks/useFilteredChatBuffer.test.tsx](../src/hooks/useFilteredChatBuffer.test.tsx) | ✅ | 8 |
| **`TwitchAdapter`** | [src/platform/twitch.test.ts](../src/platform/twitch.test.ts) | ✅ | 19 |
| **`ChzzkAdapter`** | [src/platform/chzzk.test.ts](../src/platform/chzzk.test.ts) | ✅ | 19 |

총 72 케이스, 1.21s.

## TODO — 우선순위 순

### B 단계 — 추가 순수 모듈 (DOM 무관) — ✅ 완료

- [x] **`validateFilterList`** — 8 케이스 (happy + 5 에러 코드 + 정렬 검증)
- [x] **`useFilteredChatBuffer`** — 8 케이스 (정렬 양방향 / dedupe / 트림 양방향 / clear)
  - happy-dom + @testing-library/react 추가, 파일 상단 `// @vitest-environment happy-dom` 코멘트로 per-file override
- [ ] **`useFilteredChatBuffer` persistence 케이스** — 채팅 유지 기능 도입 시 추가된 새 동작:
  - persistenceKey 없으면 storage 미접근
  - persistenceKey 있으면 마운트 시 load + restored=true로 hydrate
  - addChat 시 debounced save (500ms)
  - quota 초과 시 oldest evict + 재시도
  - 인프라: `vi.stubGlobal('browser', ...)`로 storage.local mock + `vi.useFakeTimers()` 필요
  - 예상 시간: 30-45분

### C 단계 — Platform Adapter (DOM 모킹) — ✅ 완료

- [x] **`TwitchAdapter`** — 19 케이스 (pure / URL-derived / extract / fetchBadges)
- [x] **`ChzzkAdapter`** — 19 케이스 (pure / URL-derived / prepareChatClone / extract / fetchBadges)
  - API 클라이언트는 vi.mock으로 createTwitchAPI/createChzzkAPI swap
  - DOM fixture는 happy-dom의 document로 직접 빌드
  - `window.location.pathname`은 `Object.defineProperty(window, 'location', ...)`로 per-test 교체
