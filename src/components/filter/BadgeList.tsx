...
import {
  BadgeChannelNameContext,
  BadgeListChannelContext,
  useBadgeChannelNameContext,
  useBadgeListChannelContext,
} from "../../context/BadgeChannel";
...
export default function BadgeList(props: {
  ...
  const [badgeListChannel, setBadgeListChannel] =
    React.useState<BadgeChannelType>("global");
  ...
  const {
    data: UDChannelChatBadges,
    isSuccess: isCBSuccess,
    fetchStatus: CBFetchStatus,
  } = useQuery(
...
function AddSelectedBadges(
  ...
  setAfInputRow((list) => {
    const newList: ArrayFilterInterface[] = badgesRow
      .map((badge) => {
...
function ChannelInput() {
  const { badgeListChannel, setBadgeListChannel } =
    useBadgeListChannelContext();
  const { badgeChannelName, setBadgeChannelName } =
    useBadgeChannelNameContext();
  ...
  const BadgeChannelButton = () => {
    return (
      <>
        <Button
          disabled={badgeListChannel === "global"}
          onClick={onBCButtonClicked}
        >
          {t("common.global")}
        </Button>
        <Button
          disabled={badgeListChannel === "channel"}
          onClick={onBCButtonClicked}
        >
          {t("common.channel")}
        </Button>
      ...
      {badgeListChannel === "channel" ? (
        <>
          <TextField
            ...
            size="small"
            onChange={(e) => {
              channelName.current = e.target.value;
            }}
          />
          <Button onClick={onSearchButtonClicked}>{t("common.search")}</Button>
        </>
      ) : null}
    </Stack>
  );
