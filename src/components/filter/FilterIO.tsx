...
export function ImportFilter() {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onloadend = (e: ProgressEvent<FileReader>) => {
      ...
      try {
        const filterJson: FilterJsonInterface[] | ArrayFilterListInterface[] =
          JSON.parse(e.target.result as string);

        // v1 버전의 필터 호환.
        const is_v1 = (filterJson as FilterJsonInterface[]).some(
          (f) => f.version
        );
        ...
        for (let filter of filterJson) {
          ...
          if (is_v1) {
            ...
            if (!TypeArr.includes(v2_filter.filterType)) {
              throw new Error(
                `필터 유형이 유효하지 않습니다. 필터 유형 : ${v2_filter.filterType}`
              );
            }
            if (!v2_filter.id) {
            ...
            for (let f of v2_filter.filters) {
              if (!ArrayFilterCategoryArr.includes(f.category)) {
                throw new Error(
                  `필터 카테고리 항목이 유효하지 않습니다. 필터 카테고리 : ${f.category}`
                );
              }
              if (!TypeArr.includes(f.type)) {
                throw new Error(
                  `필터 유형 항목이 유효하지 않습니다. 필터 유형 : ${f.type}`
                );
              }
              if (!f.id || !f.value) {
                throw new Error(
                  `필터에 ID 값 또는 필터 내용이 누락되었습니다.`
                );
              }
              filters.push({
                category: f.category,
