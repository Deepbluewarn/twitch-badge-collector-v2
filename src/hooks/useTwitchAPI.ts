...
export default function useTwitchAPI() {
  const fetchUser = React.useCallback(
    async (paramType?: ChannelNameType, ...reqParams: string[]) => {
  ...
  const fetchFollowedStreams = React.useCallback(
    async (userId: string, cursor?: string | null) => {
  ...
  const fetchChannelChatBadges = React.useCallback(
    async (broadcaster_id: string) => {
  ...
  const fetchUDChannelChatBadges = React.useCallback(
    async (broadcaster_id: string) => {
      const params = new URLSearchParams();
...
export interface TwitchAPIHooks {
  fetchUser: (
    paramType?: ChannelNameType,
    ...reqParams: string[]
  ) => Promise<GetUser>;
  fetchFollowedStreams: (
    user_id: string,
    after?: string | null
  ) => Promise<GetFollowedStreams>;
  fetchChannelChatBadges: (
