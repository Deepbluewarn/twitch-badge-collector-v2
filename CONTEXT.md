...
- **영상 양옆 영역 맞교환 (치지직 전용)**: [src/content-scripts/chzzk/chat-side.ts](src/content-scripts/chzzk/chat-side.ts) — 설정 `chzzkChatSide`. 기본 `[전역 내비][영상][채팅]`을 `[채팅][영상][전역 내비]`로 통째로 뒤집는다. **`position`(위/아래)과는 다른 축**: `position`은 채팅 사이드바 *안에서* 모아보기와 원본 채팅의 상하 순서다.
  ...
  - 그 폭이 펼침/접힘에 따라 달라진다(실측 240px ↔ 82px). 상수로 박거나 hash 붙은 `_is_expanded_` 클래스로 분기하는 대신 **호스트가 실제로 그린 폭을 ResizeObserver로 읽어 CSS 변수(`--tbc-sidebar-w`)로 넘긴다.** 그래서 전역 내비 스위치는 폭 측정에 성공해야만 켜진다 — 추측값으로 켜면 내비가 영상을 덮거나(과소) 오른쪽에 빈 띠가 생긴다(과다).
  - 스타일을 호스트 노드에 직접 박지 않고 `<html>` 속성/변수 + CSS로 건다. 두 aside 모두 치지직 React 소유라 reconcile 때 inline style이 날아갈 수 있지만 `<html>`은 React 트리 밖이다. 덕분에 설정 변경이 reload 없이 즉시 먹는다.
  - 전체화면 keep-alive와 충돌하지 않는다. 그쪽은 채팅 aside를 `position: absolute`로 화면 밖에 빼는데 흐름에서 빠지면 order는 무의미하고, HIDE_PROPS에 order가 없어 override 해제 시 규칙이 그대로 다시 산다.
