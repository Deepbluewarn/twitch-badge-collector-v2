...
- **채팅창 좌/우 배치 (치지직 전용)**: [src/content-scripts/chzzk/chat-side.ts](src/content-scripts/chzzk/chat-side.ts) — 설정 `chzzkChatSide`. **`position`(위/아래)과는 다른 축이다**: `position`은 사이드바 *안에서* 모아보기와 원본 채팅의 상하 순서, `chzzkChatSide`는 사이드바 *전체*를 영상 좌/우 어디에 둘지. 치지직 라이브/다시보기 모두 부모가 flex row에 [비디오, aside] 순서라 aside에 `order: -1`만 주면 뒤집힌다.
  - 스타일을 aside에 직접 박지 않고 `<html>` 속성 + CSS 규칙으로 건다. aside는 치지직 React 소유라 reconcile 때 inline style이 날아갈 수 있지만 `<html>`은 React 트리 밖이다. 덕분에 설정 변경이 reload 없이 즉시 먹고, 호스트 DOM이 뜨기 전에 미리 켜둘 수 있어 "오른쪽에 그렸다가 왼쪽으로 튀는" 깜빡임도 없다.
  - 전체화면 keep-alive와 충돌하지 않는다. 그쪽은 aside를 `position: absolute`로 화면 밖에 빼는데 흐름에서 빠지면 order는 무의미하고, HIDE_PROPS에 order가 없어 override 해제 시 규칙이 그대로 다시 산다.
- **Selectors manifest 전달 (realm 2개)**: [src/platform/host-selectors.ts](src/platform/host-selectors.ts) — 확장은 페이지당 ISOLATED(content script)와 MAIN(inject) 두 벌이 돈다. MAIN은 `browser` API가 없어 storage를 못 읽으므로, **OTA manifest가 MAIN에 닿는 유일한 경로는 ISOLATED의 `postMessage`다.** `manifestReady`는 "manifest를 받았다"가 아니라 "이제 시작해도 된다"를 뜻한다 — MAIN은 기다리지 않고 bundled로 즉시 resolve하고, 대신 수신 리스너를 세션 내내 열어둔다.
  - 예전엔 MAIN이 200ms 기다렸다가 타임아웃되면 리스너까지 제거했다. chzzk MAIN은 `document_start`, ISOLATED는 `document_idle`이라 그 사이에 HTML 파싱 전체가 들어간다 — 200ms는 늘 타임아웃됐고, 뒤늦게 온 manifest는 받는 사람이 없어 버려졌다. **OTA를 push해도 MAIN world는 영원히 bundled로 동작했고, 아무 신호도 남지 않았다.** 지금은 양방향 핸드셰이크(ISOLATED push + MAIN의 `tbcv2-selectors-request`)라 누가 먼저 뜨든 한쪽은 걸린다. 자세한 절차는 [docs/ota-selectors.md](docs/ota-selectors.md).
